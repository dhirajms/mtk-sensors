...
static unsigned char CTPM_FW_SOUL35_JUNDA[]=
{
	//#include "Ver0x0B_Beetle_FT6306_0x85_20130222_app.i"
	//#include "YARIS_M_ID85_V11_20130710_app.i"
  //      #include "Soul3.5_6306_ID85_V10_20140414_app.i"
	#include "Soul3.5_6306_ID85_V11_20140626_app.i"
};
...
E_UPGRADE_ERR_TYPE  fts_ctpm_fw_upgrade(FTS_BYTE* pbt_buf, FTS_DWRD dw_lenth)
{
	...
	for(index = 0;index < 20;index++)		//add ft6306 bootloader mode test
	{
...
int fts_ctpm_fw_upgrade_with_i_file(void)
{
	...
	if(tp_vendor == 0x5a){
		...
		pbt_buf = CTPM_FW_SOUL35_JUNDA;
		ui_sz = sizeof(CTPM_FW_SOUL35_JUNDA);
	}
...
unsigned char fts_ctpm_get_i_file_ver(void)
{
	...
	if (ui_sz > 2)
	{
		if(tp_vendor == 0x5a)
			...
			return CTPM_FW_SOUL35_JUNDA[ui_sz - 2];
	}
	else
		//TBD, error handling?
...
int fts_ctpm_auto_upg(unsigned char tp_vendor)
{
	...
	unsigned char version_list_camry_truly[] = {0x0};  //Keep the Truly tp firmware old version list that allows to be updated.
	unsigned char version_list_camry_mutto[] = {0x0};  //Keep the Mutto tp firmware old version list that allows to be updated.
	unsigned char version_list_camry_each[] = {0x0};	//Keep the ft6306 tp firmware old version list that allows to be updated.
