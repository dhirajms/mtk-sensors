...
static unsigned char *I2CDMABuf_data_va = NULL;
static volatile unsigned int I2CDMABuf_data_pa = NULL;
...
void DmaAlloc(void)
{
    if (NULL == I2CDMABuf_va)
    {
        ...
        //I2CDMABuf_va = (u8 *)dma_alloc_coherent(NULL, 255, &I2CDMABuf_pa, GFP_KERNEL);
    }
...
void DmaFree(void)
{
    if (NULL != I2CDMABuf_va)
    {
		...
		//dma_free_coherent(NULL, 255, I2CDMABuf_va, I2CDMABuf_pa);
	      I2CDMABuf_va = NULL;
	      I2CDMABuf_pa = 0;
...
void DmaAlloc_data(void)
{
    if (NULL == I2CDMABuf_data_va)
    {
        I2CDMABuf_data_va = (u8 *)dma_alloc_coherent(NULL, 255, &I2CDMABuf_data_pa, GFP_KERNEL);
    }
    
    if (NULL == I2CDMABuf_data_va)
    {
        DBG("DmaAlloc_data FAILED!");
    }
    else
    {
        DBG("DmaAlloc_data SUCCESS!");
    }
}

void DmaFree_data(void)
{
    if (NULL != I2CDMABuf_data_va)
    {
        	dma_free_coherent(NULL, 255, I2CDMABuf_data_va, I2CDMABuf_data_pa);
	      I2CDMABuf_data_va = NULL;
	      I2CDMABuf_data_pa = 0;
...
//        g_I2cClient->addr = (g_I2cClient->addr & I2C_MASK_FLAG ) | (I2C_ENEXT_FLAG);
        ...
        if (nSize > 8 && NULL != I2CDMABuf_va)
        {
	        ...
	        g_I2cClient->addr = (g_I2cClient->addr & I2C_MASK_FLAG ) | (I2C_DMA_FLAG);
...
s32 IicReadData_data(u8 nSlaveId, u8* pBuf, u16 nSize)
{
    s32 rc = 0;
...
#ifdef CONFIG_ENABLE_DMA_IIC
        if (nSize > 8 && NULL != I2CDMABuf_data_va)
        {
            s32 i = 0;
   
            g_I2cClient->addr = (g_I2cClient->addr & I2C_MASK_FLAG) | (I2C_DMA_FLAG);
            rc = i2c_master_recv(g_I2cClient, (unsigned char *)I2CDMABuf_data_pa, nSize);
        
            for (i = 0; i < nSize; i ++)
            {
                pBuf[i] = I2CDMABuf_data_va[i];
            }
        }
        else
        {
