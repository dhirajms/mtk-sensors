...
unsigned char msg2xxx_each_update_bin[94*1024] =
{
	#include "Pixi3_4_MSG2238_EACH_V5.03_140923.h"
};

unsigned char msg2xxx_green_update_bin[94*1024] =
{
	//#include "Pixi3_4_MSG2238_GREEN_V8.02_141203.h"
	//#include "Pixi3_4_MSG2238_GREEN_V8.04_141205.h"
	#include "Pixi3_4_MSG2238_GREEN_V8.05_141209.h"
};

Msg22xxSwId_e gSwId_ctpid = MSG22XX_SW_ID_UNDEFINED;
...
enum wk_wdt_type {
	WD_TYPE_NORMAL_MSTAR,
	WD_TYPE_NOLOCK_MSTAR,	
};
...
static void _DrvFwCtrlMsg22xxProgramEmem(EmemType_e eEmemType) // For MSG22XX
{
	...
	mtk_wdt_restart(WD_TYPE_NOLOCK_MSTAR);//avoid watchdog timeout
        ...
        */
        if ((pPacket[1] == 0xFF) && (pPacket[2] == 0xFF) && (pPacket[3] == 0xFF) && (pPacket[4] == 0xFF) && (pPacket[6] == 0xFF))
        {
			...
			if(mstar2238_pls_opened)
			{
				...
				printk("fj: mstar2238_pls_opened = ture \n");
				if(0x80 == pPacket[5])  //near
				{					
					printk("fj: pPacket[5]=0x80  near \n");
					g2238ps_state = 0;
					sensor_data.values[0] = g2238ps_state;
				}
				else if (0x40 == pPacket[5]) //far
				{	
					printk("fj: pPacket[5]=0x40  far \n");
					g2238ps_state = 1;
					sensor_data.values[0] = g2238ps_state;
					msg2238_tp_resume_flag=1;
				}
				...
				{
					printk("fj: call hwmsen_get_interrupt_data fail = %d\n", ret);
				}
				printk("fj: ps_state =%d\n", g2238ps_state);//add zy
				
				if(pPacket[5]==0x80 || pPacket[5]==0x40)
				{
					printk("fj: pPacket[5] retrun -1 \n");
					return -1; 
				}
			}
		#endif
    ...
    else if (pPacket[nCheckSumIndex] == nCheckSum && pPacket[0] == 0x62)
    {
        ...
        if ((pPacket[1] == 0xFF) && (pPacket[2] == 0xFF) && (pPacket[3] == 0xFF) && (pPacket[4] == 0xFF) && (pPacket[5] == 0xFF) && (pPacket[6] == 0xFF) && (pPacket[7] == 0xFF))
        {
            ...
            {   /* 0x00 is key up, 0xff is touch screen up */
                ...
                if (pPacket[8] == 1) // TOUCH_KEY_BACK
                {
                    pInfo->tPoint[0].nX = g_TpVirtualKeyDimLocal[0][0];
                    pInfo->tPoint[0].nY = g_TpVirtualKeyDimLocal[0][1];
                }
                else if (pPacket[8] == 2) //TOUCH_KEY_HOME 
                {
                    pInfo->tPoint[0].nX = g_TpVirtualKeyDimLocal[1][0];
                    pInfo->tPoint[0].nY = g_TpVirtualKeyDimLocal[1][1];
                }           
                else if (pPacket[8] == 4) // TOUCH_KEY_MENU
                {
                    pInfo->tPoint[0].nX = g_TpVirtualKeyDimLocal[2][0];
                    pInfo->tPoint[0].nY = g_TpVirtualKeyDimLocal[2][1];
                }           
...
void _DrvFwCtrlMsg22xxCheckFirmwareUpdateBySwId(void) // For MSG22XX
{
	...
	gSwId_ctpid = eSwId; //add by fangjie, gSwId_ctpid for rawdata test.
        if (eSwId == MSG22XX_SW_ID_EACH)
        {
            nUpdateBinMajor = msg2xxx_each_update_bin[0xBFF5]<<8 | msg2xxx_each_update_bin[0xBFF4];
            nUpdateBinMinor = msg2xxx_each_update_bin[0xBFF7]<<8 | msg2xxx_each_update_bin[0xBFF6];
        }
        else if (eSwId == MSG22XX_SW_ID_GREEN)
        {
            nUpdateBinMajor = msg2xxx_green_update_bin[0xBFF5]<<8 | msg2xxx_green_update_bin[0xBFF4];
            nUpdateBinMinor = msg2xxx_green_update_bin[0xBFF7]<<8 | msg2xxx_green_update_bin[0xBFF6];
        }
            ...
            if (eSwId < MSG22XX_SW_ID_UNDEFINED && eSwId != 0x0000 && eSwId != 0xFFFF)
            {
                if (eSwId == MSG22XX_SW_ID_EACH)
                {
                    for (i = 0; i < (MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE+1); i ++)
                    {
                        if (i < MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE) // i < 48
                        {
                            _DrvFwCtrlStoreFirmwareData(&(msg2xxx_each_update_bin[i*1024]), 1024);
                        }
                        else // i == 48
                        {
                            _DrvFwCtrlStoreFirmwareData(&(msg2xxx_each_update_bin[i*1024]), 512);
                        }
                    }
                }
                else if (eSwId == MSG22XX_SW_ID_GREEN)
                {
                    for (i = 0; i < (MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE+1); i ++)
                    {
                        if (i < MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE) // i < 48
                        {
                            _DrvFwCtrlStoreFirmwareData(&(msg2xxx_green_update_bin[i*1024]), 1024);
                        }
                        else // i == 48
                        {
                            _DrvFwCtrlStoreFirmwareData(&(msg2xxx_green_update_bin[i*1024]), 512);
                        }
                    }
                }
...
void _DrvFwCtrlMsg21xxaCheckFirmwareUpdateBySwId(void) // For MSG21XXA
{
    ...
    if (nCrcMainA == nCrcMainB) 
    {
        ...
        if (eSwId == MSG21XXA_SW_ID_EACH)
        {
#ifdef CONFIG_UPDATE_FIRMWARE_BY_TWO_DIMENSIONAL_ARRAY // By two dimensional array
            nUpdateBinMajor = msg2xxx_each_update_bin[31][0x34F]<<8 | msg2xxx_each_update_bin[31][0x34E];
            nUpdateBinMinor = msg2xxx_each_update_bin[31][0x351]<<8 | msg2xxx_each_update_bin[31][0x350];
#else // By one dimensional array
            nUpdateBinMajor = msg2xxx_each_update_bin[0x7F4F]<<8 | msg2xxx_each_update_bin[0x7F4E];
            nUpdateBinMinor = msg2xxx_each_update_bin[0x7F51]<<8 | msg2xxx_each_update_bin[0x7F50];
#endif
        }
        else if (eSwId == MSG21XXA_SW_ID_GREEN)
        {
#ifdef CONFIG_UPDATE_FIRMWARE_BY_TWO_DIMENSIONAL_ARRAY // By two dimensional array
            nUpdateBinMajor = msg2xxx_green_update_bin[31][0x34F]<<8 | msg2xxx_green_update_bin[31][0x34E];
            nUpdateBinMinor = msg2xxx_green_update_bin[31][0x351]<<8 | msg2xxx_green_update_bin[31][0x350];
#else // By one dimensional array
            nUpdateBinMajor = msg2xxx_green_update_bin[0x7F4F]<<8 | msg2xxx_green_update_bin[0x7F4E];
            nUpdateBinMinor = msg2xxx_green_update_bin[0x7F51]<<8 | msg2xxx_green_update_bin[0x7F50];
#endif
        }
        ...
        if ((nUpdateBinMinor > nMinor && nIsCompareVersion == 1) || (nIsCompareVersion == 0))
        {
            if (eSwId < MSG21XXA_SW_ID_UNDEFINED && eSwId != 0xFFFF)
            {
                if (eSwId == MSG21XXA_SW_ID_EACH)
                {
                    ...
                    {
#ifdef CONFIG_UPDATE_FIRMWARE_BY_TWO_DIMENSIONAL_ARRAY // By two dimensional array
                        _DrvFwCtrlStoreFirmwareData(msg2xxx_each_update_bin[i], 1024);
#else // By one dimensional array
                        _DrvFwCtrlStoreFirmwareData(&(msg2xxx_each_update_bin[i*1024]), 1024);
#endif
                    }
                ...
                else if (eSwId == MSG21XXA_SW_ID_GREEN)
                {
                    ...
                    {
#ifdef CONFIG_UPDATE_FIRMWARE_BY_TWO_DIMENSIONAL_ARRAY // By two dimensional array
                        _DrvFwCtrlStoreFirmwareData(msg2xxx_green_update_bin[i], 1024);
#else // By one dimensional array
                        _DrvFwCtrlStoreFirmwareData(&(msg2xxx_green_update_bin[i*1024]), 1024);
#endif
                    }
...
//BEGIN:  add for Rawdata test, report point data use the different DMA buffer.
#ifdef CONFIG_ENABLE_DMA_IIC
    DmaAlloc_data(); 
#endif //CONFIG_ENABLE_DMA_IIC
    //old: IicReadData(SLAVE_I2C_ID_DWI2C, &pPacket[0], nReportPacketLength);
    IicReadData_data(SLAVE_I2C_ID_DWI2C, &pPacket[0], nReportPacketLength);
#ifdef CONFIG_ENABLE_DMA_IIC
    DmaFree_data();
#endif //CONFIG_ENABLE_DMA_IIC
//END:  add for Rawdata test, report point data use the different DMA buffer.
#endif
    ...
    if (0 == _DrvFwCtrlParsePacket(pPacket, nReportPacketLength, &tInfo))
    {
        ...
        else //touch on screen
        {
            ...
            {
#ifdef CONFIG_TP_HAVE_KEY
                if (tInfo.nTouchKeyCode == 1) // TOUCH_KEY_BACK
                {
                    nTouchKeyCode = g_TpVirtualKey[0];           
                }
                else if (tInfo.nTouchKeyCode == 2) // TOUCH_KEY_HOME
                {
