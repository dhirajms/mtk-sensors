...
//  .address_data = &addr_data,
 ...
 static int ft3306_write_reg(u8 addr, u8 para)
{
	u8 buf[2];
	int ret = -1;
...
static int ft3306_read_reg(u8 addr, u8 *pdata)
{
	int ret;
	u8 buf[1];
...
{
	//#include "Ver0x0B_Beetle_FT3306_0x80_20130222_app.i"
...
{
	//#include "Ver0x0B_Beetle_FT3306_0x85_20130222_app.i"
...
static unsigned char ft3306_read_fw_ver(void)
{
	unsigned char ver;
	ft3306_read_reg(0xA6, &ver);
	return(ver);
}
...
{ 
	...
	else
	{
		...
		//TPD_DMESG("[FT3306] I2C  DMA transfer----byte_write------llf\n");
	...
	if(ret<=0)
	{
		TPD_DMESG("[FT3306]i2c_write_byte error line = %d, ret = %d\n", __LINE__, ret);
		return FTS_FALSE;
	}
	return FTS_TRUE;
}
...
FTS_BOOL byte_read(FTS_BYTE* pbt_buf, FTS_BYTE bt_len)
{
	...
	if(bt_len <= 8)
	{
		...
		{
			i2c_clientft->addr=addr;
			TPD_DMESG("[FT3306]i2c_read_byte error line = %d, ret = %d\n", __LINE__, ret);
			return FTS_FALSE;
		}
	}
	else
	{
		...
		//TPD_DMESG("[FT3306] I2C  DMA transfer-------byte_read-------llf\n");
		mutex_lock(&dma_model_mutex);
		i2c_clientft->addr = i2c_clientft->addr & I2C_MASK_FLAG | I2C_DMA_FLAG;
		mutex_unlock(&dma_model_mutex);
...
E_UPGRADE_ERR_TYPE  fts_ctpm_fw_upgrade(FTS_BYTE* pbt_buf, FTS_DWRD dw_lenth)
{
	...
	dma_buffer= (u8 *)dma_alloc_coherent(NULL, 134, &dma_handle, GFP_KERNEL);
    	if(!dma_buffer)
	{
    		printk("[FT3306][TSP] dma_alloc_coherent error\n");
	}
	...
	for(index = 0; index <= FTS_UPGRADE_LOOP; index++)
	{
		...
		TPD_DMESG("[FT3306-upgrade] Step 2: Enter upgrade mode\n");
		...
		byte_read(reg_val,2);
		TPD_DMESG("[FT3306-upgrade] Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",reg_val[0],reg_val[1]);
		if (reg_val[0] == 0x79 && reg_val[1] == 0x08)
		{
			TPD_DMESG("[FT3306] check READ-ID Successful ! index = %d------\n", index);
			break;
		}
		else if(index >= FTS_UPGRADE_LOOP)
		{
			TPD_DMESG("[FT3306] check READ-ID faild !\n");
			i2c_clientft->timing = 100;
			return ERR_READID;
		}
	}	
	printk("[FT3306]---index: %d------\n", index);
	cmd_write(0xcd,0x0,0x00,0x00,1);
	byte_read(reg_val,1);
	TPD_DMESG("[FT3306-upgrade] bootloader version = 0x%x\n", reg_val[0]);
	...
	delay_qt_ms(100);
	TPD_DMESG("[FT3306-upgrade] Step 4: erase. \n");

	/*********Step 5:write firmware(FW) to ctpm flash*********/
	bt_ecc = 0;
	TPD_DMESG("[FT3306-upgrade] Step 5: start upgrade. \n");
	dw_lenth = dw_lenth - 8;
	...
	for (j = 0;j < packet_number;j ++)
	{
		...
		{
			TPD_DMESG("[FT3306-upgrade] upgrade the 0x%x th byte.\n", ((unsigned int)j) * FTS_PACKET_LENGTH);
		}
	}
	...
	byte_read(reg_val,1);
	TPD_DMESG("[FT3306-upgrade] Step 6:  ecc read 0x%x, new firmware 0x%x. \n", reg_val[0], bt_ecc);
	if(reg_val[0] != bt_ecc)
	{
		i2c_clientft->timing = 100;
		return ERR_ECC;
...
int fts_ctpm_auto_clb(void)
{
	...
	TPD_DMESG("[FT3306] start auto CLB.\n");
	msleep(200);
	ft3306_write_reg(0, 0x40);  
	delay_qt_ms(100);   //make sure already enter factory mode=Test mode
	ft3306_write_reg(2, 0x4);  //write command to start calibration
	delay_qt_ms(300);
	
	for(i=0;i<50;i++) //llf 20130220
	{
		uc_temp=0x0;
		ft3306_read_reg(0x0,&uc_temp);
		...
		delay_qt_ms(200);
		//mtk_wdt_restart(WK_WDT_EXT_TYPE_NOLOCK);//avoid watchdog timeout
		TPD_DMESG("[FT3306] waiting calibration %d\n",i);
	}
	mtk_wdt_restart(WK_WDT_EXT_TYPE_NOLOCK);//avoid watchdog timeout
	msleep(200);
	ft3306_write_reg(0, 0x40);  
	...
	TPD_DMESG("[FT3306] calibration OK.\n");

	msleep(300);
	ft3306_write_reg(0, 0x40);  //goto factory mode
	delay_qt_ms(100);   //make sure already enter factory mode
	ft3306_write_reg(2, 0x5);  //store CLB result
	delay_qt_ms(300);
	ft3306_write_reg(0, 0x0); //return to normal mode 
	msleep(300);
	TPD_DMESG("[FT3306] store CLB result OK.\n");
...
int fts_ctpm_get_panel_factory_setting(void)    //return factory ID
{
	...
	TPD_DMESG("[FT3306] Enter fts_ctpm_get_panel_factory_setting to get factory ID\n");

	i2c_clientft->timing = 400;       //when in upgrade mode ,the clock should be changed to 400
	/*********Step 1:Reset  CTPM *****/
	for(j = 0;j < sizeof(reg_addr)/sizeof(reg_addr[0]);j++)		//add FT3306 bootloader mode test
	{
		/*write 0xaa to register 0xfc(FT3306) or 0xbc(FT3306)*/
		ft3306_write_reg(reg_addr[j],0xaa);
		delay_qt_ms(delay_time1[j]);
		/*write 0x55 to register 0xfc(FT3306) or 0xbc(FT3306)*/
		ft3306_write_reg(reg_addr[j],0x55);
		TPD_DMESG("[FT3306] Step 1: Reset CTPM test\n");

		delay_qt_ms(delay_time2[j]);

		/*********Step 2:Enter upgrade mode *****/
		TPD_DMESG("[FT3306] Step 2: Enter upgrade mode\n");
		auc_i2c_write_buf[0] = 0x55;
		auc_i2c_write_buf[1] = 0xaa;
		do
		{
...
*******************************************/
	...
	TPD_DMESG("[FT3306] The buf[0] is 0x%x\n",buf[0]);	
//set read start address
	...
	TPD_DMESG("[FT3306] old setting: fw id = 0x%x\n",buf[0]);
	tp_fw_ver = buf[0];
	TPD_DMESG("[FT3306] old setting: tp_fw_ver = 0x%x\n",tp_fw_ver);
...
int fts_ctpm_auto_upg(unsigned char tp_vendor)
{
	...
	unsigned char version_list_camry_each[] = {0x0,0x10,0x11,0x12,0x13,0x14,0x15,0x16,0x20,0x61};	//Keep the FT3306 tp firmware old version list that allows to be updated.
//	unsigned char version_list_camry_junda[] = {0x0,0x10,0x11,0x12,0x13,0x14,0x15};	//Keep the FT3306 tp firmware old version list that allows to be updated.
	unsigned char version_list_camry_junda[] = {0x0,0x0A,0x0B,0x0C,0x0D,0x0E,0x0F,0x10,0x11,0x12,0x13,0x14,0x15,0x16,0x17,0x18,0x19,0x30,0x31,0x32,0x33,0x34,0x35};	//Keep the FT3306 tp firmware old version list that allows to be updated.
	int i,i_ret = -1;

	uc_tp_fm_ver = ft3306_read_fw_ver();
	if(tp_vendor == 0x5a){
		uc_host_fm_ver = fts_ctpm_get_i_file_ver();
		TPD_DMESG("[FT3306] tp vendor is Truly(0x%x),uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",tp_vendor, uc_tp_fm_ver, uc_host_fm_ver);
		for (i = 0; i < sizeof(version_list_camry_truly)/sizeof(version_list_camry_truly[0]); i++)
		{
			if(uc_tp_fm_ver<uc_host_fm_ver)//llf 20130222
			{
				if (uc_tp_fm_ver == version_list_camry_truly[i])
	       	 		{
	       	 			TPD_DMESG("[FT3306]  tp firmware have new version \n");
		...
		uc_host_fm_ver = fts_ctpm_get_i_file_ver();
		TPD_DMESG("[FT3306] tp vendor is Mutto(0x%x),uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",tp_vendor, uc_tp_fm_ver, uc_host_fm_ver);
		for (i = 0; i < sizeof(version_list_camry_mutto)/sizeof(version_list_camry_mutto[0]); i++)
		{
			if(uc_tp_fm_ver<uc_host_fm_ver)//llf 20130222
			{
				if (uc_tp_fm_ver == version_list_camry_mutto[i])
					...
					i_ret = fts_ctpm_fw_upgrade_with_i_file();
					if (!i_ret )
						TPD_DMESG("[FT3306] Mutto tp firmware upgrade to new version 0x%x\n", uc_host_fm_ver);
					else
						TPD_DMESG("[FT3306] Mutto tp firmware upgrade failed ret=%d.\n", i_ret);
					break;
				}
			}
			else
		...
		uc_host_fm_ver = fts_ctpm_get_i_file_ver();
		TPD_DMESG("[FT3306] tp vendor is unkown(0x%x),uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",tp_vendor, uc_tp_fm_ver, uc_host_fm_ver);
		...
		//		if(uc_tp_fm_ver == version_list_camry_each[i])  
				{
					TPD_DMESG("[FT3306] Each tp firmware have new version\n");
					i_ret = fts_ctpm_fw_upgrade_with_i_file();
					if(!i_ret)
						TPD_DMESG("[FT3306] Each tp firmware upgrade to new version 0x%x\n", uc_host_fm_ver);
					else
						TPD_DMESG("[FT3306] Each tp firmware upgrade failed ret=%d.\n", i_ret);
					break;
				}
			}
			else
			{
				TPD_DMESG("[FT3306] current Each tp firmware is the latest version!\n");
				break;
			}
		}
	}
		...
		for(i = 0;i < sizeof(version_list_camry_junda)/sizeof(version_list_camry_junda[0]); i++)
		{
			...
			//if(1) //force to up
			{
				...
				{
					TPD_DMESG("[FT3306] Junda tp firmware have new version\n");
					i_ret = fts_ctpm_fw_upgrade_with_i_file();
					if(!i_ret)
						TPD_DMESG("[FT3306] Junda tp firmware upgrade to new version 0x%x\n", uc_host_fm_ver);
					else
						TPD_DMESG("[FT3306] Junda tp firmware upgrade failed ret=%d.\n", i_ret);
					break;
				}
			}
			else
			{
				TPD_DMESG("[FT3306] current Junda tp firmware is the latest version!\n");
				break;
			}
		}
	}
	else{
		TPD_DMESG("[FT3306] Unknown tp vendor(0x%x).\n", uc_tp_fm_ver);
		return -1;
	}
    return 0;
}
...
static int fps_state = -1;
static int ft3306_tp_resume_flag=0;//once the tp reset , should set tp_resume_flag=0
//extern int tp_pls_status;
static int FT3306_pls_opened=0;
static void Ft3306_tp_reset()
{
	mt_set_gpio_mode(GPIO_CTP_EN_PIN, GPIO_CTP_EN_PIN_M_GPIO);
	mt_set_gpio_dir(GPIO_CTP_EN_PIN, GPIO_DIR_OUT);
	mt_set_gpio_out(GPIO_CTP_EN_PIN, GPIO_OUT_ONE);
	...
	mt_set_gpio_out(GPIO_CTP_EN_PIN, GPIO_OUT_ONE);
	msleep(300);	
	ft3306_tp_resume_flag=0;
}
/*************************************************************************************/

static void  FT3306_ps_mode_enable(bool enable)
{	      
	int err = 0;
	unsigned char rx_data;
	if(enable)  //enter phone open
	{
		err= ft3306_write_reg(0xB0,0x01);
		TPD_DMESG("[FT3306_PS]open ps is %d \n",err);
		err=ft3306_read_reg(0xB0,&rx_data);
		if(err>=0 && (rx_data&0x01>0))
			TPD_DMESG("[FT3306_PS]open ps OK\n");
		else
			TPD_DMESG("[FT3306_PS]open ps failed\n");
	}
	else //close phone
	{     
		err= ft3306_write_reg(0xB0,0x00);
		TPD_DMESG("[FT3306_PS]close ps is %d \n",err);
		err=ft3306_read_reg(0xB0,&rx_data);
		if(err>=0 && (rx_data&0x01==0))
			TPD_DMESG("[FT3306_PS]close ps OK\n");
		else
			TPD_DMESG("[FT3306_PS]close ps failed\n");
	}
}

int FT3306_pls_enable(void)
{
	TPD_DMESG("[FT3306_PS]FT3306_pls_enable\n");
	FT3306_ps_mode_enable(true);
	FT3306_pls_opened = 1;
	return 0;
}
 
int FT3306_pls_disable(void)
{
	TPD_DMESG("[FT3306_PS]FT3306_pls_disable\n");
	FT3306_ps_mode_enable(false);
	FT3306_pls_opened = 0;
	return 0;
}	

int  get_FT3306_data(void )
{
	return fps_state;
}

 static int FT3306_pls_open(struct inode *inode, struct file *file)
 {
	 TPD_DMESG("[FT3306_PS] %s\n", __func__);
	 if (FT3306_pls_opened)
		 return -EBUSY;
	 FT3306_pls_opened = 1;
	 return 0;
 }
 static int FT3306_pls_release(struct inode *inode, struct file *file)
 {
	 TPD_DMESG("[FT3306_PS] %s", __func__);
	 FT3306_pls_opened = 0;
	 return 0;//ap3212c_pls_disable(AP3212C_PLS_BOTH);
 }

 static long FT3306_pls_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
 {
	 void __user *argp = (void __user *)arg;
	 TPD_DMESG("[FT3306_PS] %s: cmd %d", __func__, _IOC_NR(cmd));
	 switch (cmd) {
	 case 1:
		 TPD_DMESG("[FT3306_PS] %s: FACE_DETECT_NEAR*************************\n", __func__);
		 FT3306_pls_enable();
		 break;
	 case 2:
		 TPD_DMESG("[FT3306_PS] %s: FACE_DETECT_FAR*************************\n", __func__);
		 FT3306_pls_disable();
		 break;
	 default:
		 TPD_DMESG("[FT3306_PS] %s: invalid cmd %d\n", __func__, _IOC_NR(cmd));
		 return -EINVAL;
	 }
	 return 0;
 }
 
 static struct file_operations FT3306_pls_fops = {
	 .owner 			 = THIS_MODULE,
	 .open			 = FT3306_pls_open,
	 .release			 = FT3306_pls_release,
	 .unlocked_ioctl 			 = FT3306_pls_ioctl,
 };
 static struct miscdevice FT3306_pls_device = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = ALSPS_DEVICE_NAME,
	.fops = &FT3306_pls_fops,
};
#endif
...
#ifdef APK_UPGRADE
static int ft3306_GetFirmwareSize(char *firmware_name)
{
	...
	printk("[FT3306_llf] Get firmware size begain\n");
...
static int ft3306_ReadFirmware(char *firmware_name,
			       unsigned char *firmware_buf)
{
	...
	printk("[FT3306_llf] Read firmware begain!\n");
	memset(filepath, 0, sizeof(filepath));
	sprintf(filepath, "%s", firmware_name);
	if (NULL == pfile)
		pfile = filp_open(filepath, O_RDONLY, 0);
				       ...
				       char *firmware_name)
{
	u8 *pbt_buf = NULL;
	int i_ret;
	int fwsize = ft3306_GetFirmwareSize(firmware_name);

	printk("[FT3306_llf] Get firmware size ok size=%d \n",fwsize);
	if (fwsize <= 0) {
	...
	if (ft3306_ReadFirmware(firmware_name, pbt_buf)) {
		dev_err(&client->dev, "%s() - ERROR: request_firmware failed\n",__func__);
		kfree(pbt_buf);
		return -EIO;
	}
	printk("[FT3306_llf] Get firmware file ok\n");

	printk("[FT3306_llf] File just is 0x%x 0x%x 0x%x \n",(pbt_buf[fwsize - 8] ^ pbt_buf[fwsize - 6]),
	...
	if ((pbt_buf[fwsize - 8] ^ pbt_buf[fwsize - 6]) == 0xFF
		...
		/*call the upgrade function */
		printk("[FT3306_llf] Begain to Upgrade firmware \n");
		...
		printk("[FT3306_llf] Firmware file format error\n");
		kfree(pbt_buf);
		return -EIO;
	}
	return i_ret;
...
static ssize_t ft3306_fwupgradeapp_show(struct device *dev,struct device_attribute *attr,char *buf)
{
...
static ssize_t ft3306_fwupgradeapp_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
	char fwname[128];
	...
	printk("[FT3306_llf]Input file name is %s \n",fwname);
	...
	fm_ver = ft3306_read_fw_ver();
	//i2c_smbus_read_i2c_block_data(i2c_clientft, 0xA6, 1, &fm_ver);
	printk("[FT3306_llf] current firmware version is  0x%x\n", fm_ver);
	
	ft3306_read_reg(0xA8, &tp_vendor);
	...
	if((tp_vendor != 0x5a) && (tp_vendor != 0x53) && (tp_vendor != 0x80)&& (tp_vendor != 0x85)){
		//destory #######################################
		mt65xx_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
		printk("[%s] can't find FT3306,vendor:0x%x\n",__func__,tp_vendor);
		return -1;
	}
	printk("FT3306_llf] FT3306 vendor is 0x%x\n", tp_vendor);
	...
	printk("[FT3306_llf] Firmware Upgrade Successful!\n");

	fm_ver = ft3306_read_fw_ver();
	
	printk("[FT3306_llf]  After Upgrade firmware version is  0x%x\n", fm_ver);
...
*/
static DEVICE_ATTR(ftsfwupgradeapp, S_IRUGO | S_IWUSR, ft3306_fwupgradeapp_show,
			ft3306_fwupgradeapp_store);
 ...
 static int touch_event_handler(void *unused)
 {
	...
	tpd_flag = 0;		 
 	...
 	if(FT3306_pls_opened){
		...
		ret=ft3306_read_reg(0x01, &ps_data);

		printk("[FT3306_llf] PS reg data =0x%x \n ",ps_data);
...
//static DEVICE_ATTR(ftsrawdatashow, S_IRUGO | S_IWUSR, ft3306_rawdata_show,
			//ft3306_rawdata_store);
...
static struct attribute *ft3306_attributes[] = {
	//&dev_attr_ftsrawdatashow.attr,
        &dev_attr_ftsfwupgradeapp.attr,
	NULL
};

static struct attribute_group ft3306_attribute_group = {
	.attrs = ft3306_attributes
};
...
{ 
	...
	return sysfs_create_group(TP_ctrl_kobj, &ft3306_attribute_group);
}
//remove sysfs
void TP_sysfs_exit(void)
{
	sysfs_remove_group(TP_ctrl_kobj, &ft3306_attribute_group);
			  ...
			  char *buf)
{
	...
	if (ft3306_read_reg(0xa6, &chip_fw_ver) < 0) {
		printk("read chip version is error\n");
	}
	
	if((ft3306_read_reg(0xa8, &tp_factory_id_cmd))< 0)
	    {
...
#define tp3306switch_ctrl_attr(_name) \
...
tp3306switch_ctrl_attr(name);
tp3306switch_ctrl_attr(tp_value);
...
/*
	...
	if((tp_vendor == 0x5a) || (tp_vendor == 0x53) || (tp_vendor == 0x80)|| (tp_vendor == 0x85))
	{
			tpd_load_status = 1;
			//tp_pls_status = 1;
			TPD_DMESG("[FT3306][The CTP is FT3306 ---llf] 0xA8 current version value is  0x%x\n", tp_vendor);
	}
	else
	{
			tpd_load_status =0;
			TPD_DMESG("[FT3306][The CTP is MSG2133 ---llf] 0xA8 current version value is  0x%x\n", tp_vendor);
	}
*/
	...
	if((tp_vendor != 0x5a) && (tp_vendor != 0x53) && (tp_vendor != 0x80)&& (tp_vendor != 0x85)){
		//destory #######################################
		mt65xx_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
		TPD_DMESG("[%s] can't find FT3306,vendor:0x%x\n",__func__,tp_vendor);
		return -1;
	}
//	fts_ctpm_get_panel_factory_setting();
  ...
  #endif
#ifdef CTP_PSENSOR_SUPPORT
	ret = misc_register(&FT3306_pls_device);
	if (ret != 0) {
		printk("cannot register miscdev on FT3306_pls_device\n");
	}
#endif
...
//	}
 ...
 {
	TPD_DMESG("[FT3306]--------------------------------------tpd_local_init\n");
	...
	{
		TPD_DMESG("[FT3306] add i2c driver faild.\n");
 ...
 static void tpd_resume( struct early_suspend *h )
 {
	...
	if(FT3306_pls_opened)
	{	
		if(ft3306_tp_resume_flag==0){
			Ft3306_tp_reset();
		}
		FT3306_ps_mode_enable(true);
		printk("==%s= ft3306_pls_opened ! return\n", __func__);
		mt65xx_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM); 
		return;
	}
#endif
 ...
 static int __init tpd_driver_init(void) {
	TPD_DMESG("MediaTek FT3306 touch panel driver init\n");
	i2c_register_board_info(TPD_I2C_NUMBER, &i2c_tpd, 1);
	if(tpd_driver_add(&tpd_device_driver) < 0)
		TPD_DMESG("add FT3306 driver failed\n");
	return 0;
 }
 
 /* should never be called */
 static void __exit tpd_driver_exit(void) {
	 TPD_DMESG("MediaTek FT3306 touch panel driver exit\n");
