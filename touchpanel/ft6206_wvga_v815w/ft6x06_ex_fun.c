...
//	#include "gelinF040-48-0X03-LECHIN_20140124_app.i"
};

// CTP1
static unsigned char CTPM_FW_CTP1[] = {
	#include "IT_TMK_T120397_ZTE_V815W_V14_20140617_app.i"
};

//CTP2
static unsigned char CTPM_FW_CTP2[] = {
	#include "Ft6206_CengHong_TP301440B_Ver02_20140418_app.i"
};
/*
...
int fts_ctpm_fw_upgrade_with_i_file_ctp1(struct i2c_client *client)
{
	u8 *pbt_buf = NULL;
	int i_ret;
	int fw_len = sizeof(CTPM_FW_CTP1);
	...
	*/
	...
	if (((CTPM_FW_CTP1[fw_len - 8] ^ CTPM_FW_CTP1[fw_len - 6]) == 0xFF)
		&& ((CTPM_FW_CTP1[fw_len - 7] ^ CTPM_FW_CTP1[fw_len - 5]) == 0xFF)
		&& ((CTPM_FW_CTP1[fw_len - 3] ^ CTPM_FW_CTP1[fw_len - 4]) == 0xFF)) 
       {
		/*FW upgrade */
		pbt_buf = CTPM_FW_CTP1;
		/*call the upgrade function */
		i_ret = fts_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_CTP1));
		if (i_ret != 0)
			{
			dev_err(&client->dev, "%s:upgrade failed. err.\n",__func__);
			}
...
int fts_ctpm_fw_upgrade_with_i_file_ctp2(struct i2c_client *client)
{
	u8 *pbt_buf = NULL;
	int i_ret;
	int fw_len = sizeof(CTPM_FW_CTP2);
	...
	*/
	...
	if (((CTPM_FW_CTP2[fw_len - 8] ^ CTPM_FW_CTP2[fw_len - 6]) == 0xFF)
		&& ((CTPM_FW_CTP2[fw_len - 7] ^ CTPM_FW_CTP2[fw_len - 5]) == 0xFF)
		&& ((CTPM_FW_CTP2[fw_len - 3] ^ CTPM_FW_CTP2[fw_len - 4]) == 0xFF)) 
       {
		/*FW upgrade */
		pbt_buf = CTPM_FW_CTP2;
		/*call the upgrade function */
		i_ret = fts_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_CTP2));
		if (i_ret != 0)
			{
			dev_err(&client->dev, "%s:upgrade failed. err.\n",__func__);
			}
...
u8 fts_ctpm_get_i_file_ver_ctp1(void)
{
	u16 ui_sz;
	ui_sz = sizeof(CTPM_FW_CTP1);
	if (ui_sz > 2)
		return CTPM_FW_CTP1[ui_sz - 2];

	return 0x00;	/*default value */
}
u8 fts_ctpm_get_i_file_ver_ctp2(void)
{
	u16 ui_sz;
	ui_sz = sizeof(CTPM_FW_CTP2);
	if (ui_sz > 2)
		return CTPM_FW_CTP2[ui_sz - 2];
...
int fts_ctpm_enter_upgrade_mode(struct i2c_client *client)
{
	u8 reg_val[20] = {0};
	u32 i = 0;
	u32 packet_number;
