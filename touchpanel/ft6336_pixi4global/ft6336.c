...
#include "tpd_custom_ft6336_pixi4global.h"
...
//#define FTS_CTL_IIC //fangjie delete
...
#define TPD_DEVICEFT "FT6336"
#define FTS_UPGRADE_LOOP  10

#define FT_TAG                  "FT6336"
#define FT_FUN(f)               printk(KERN_INFO FT_TAG"%s\n", __FUNCTION__)
#define FT_ERR(fmt, args...)    printk(KERN_ERR FT_TAG"%s %d : "fmt, __FUNCTION__, __LINE__, ##args)
#define FT_LOG(fmt, args...)    printk(KERN_INFO FT_TAG fmt, ##args)
#define TPD_OK 0
//#define TPD_CLOSE_POWER_IN_SLEEP //fangjie close, because TP will inavaild after resume.
...
typedef enum {
 ...
 //BEGIN: modify by fangjie, Because since KK, the watchdog restart type only include WD_TYPE_NORMAL and WD_TYPE_NOLOCK.
#if 0
enum wk_wdt_type {
	WK_WDT_LOC_TYPE,
	WK_WDT_EXT_TYPE,
...
//BEGIN: fangjie move from macro FT_BOOT_UPGRADE_FIRMWARE to the outside for MMITest rawdata test.
typedef unsigned char         FTS_BYTE;     //8 bit
static FTS_BYTE *dma_buffer = NULL;//FTS_BYTE=u8;
static u32 dma_handle = NULL;
static struct mutex dma_model_mutex;
//END: fangjie move from macro FT_BOOT_UPGRADE_FIRMWARE to the outside for MMITest rawdata test.
 

static unsigned char CTPM_FW_PIXI4_JUNDA[]=
{
	//#include "Soul4_6336_ID85_V11_20140605_app.i"
	//#include "PIXI3_4JD85_V10_FT6336_20140825_app.i"
	//#include "Pixi3_4_6336_ID85_V11_20140911_app.i"
	//#include "Pixi3_4_6336_ID85_V14_20141105_app.i"
	//#include "Pixi3_4_6336_ID85_V15_20141120_app.i" 
	//#include "Pixi3_4_6336_ID85_V16_20141121_app.i"
	#include "Pixi3_4_6336_ID85_V17_20141205_app.i"  //add by fangjie for PR864101 
...
{ 
	...
	else
	{
		...
		//TPD_DMESG("[FT6336] I2C  DMA transfer----byte_write------llf\n");
	...
	if(ret<=0)
	{
		TPD_DMESG("[FT6336]i2c_write_byte error line = %d, ret = %d\n", __LINE__, ret);
		return FTS_FALSE;
	}
	return FTS_TRUE;
}
...
FTS_BOOL byte_read(FTS_BYTE* pbt_buf, FTS_BYTE bt_len)
{
	...
	if(bt_len <= 8)
	{
		...
		{
			i2c_clientft->addr=addr;
			TPD_DMESG("[FT6336]i2c_read_byte error line = %d, ret = %d\n", __LINE__, ret);
			return FTS_FALSE;
		}
	}
	else
	{
		//TPD_DMESG("[FT6336] I2C  DMA transfer-------byte_read-------llf\n");
		mutex_lock(&dma_model_mutex);
		i2c_clientft->addr = i2c_clientft->addr & I2C_MASK_FLAG | I2C_DMA_FLAG;
		mutex_unlock(&dma_model_mutex);
...
E_UPGRADE_ERR_TYPE  fts_ctpm_fw_upgrade(FTS_BYTE* pbt_buf, FTS_DWRD dw_lenth)
{
	...
	dma_buffer= (u8 *)dma_alloc_coherent(NULL, 134, &dma_handle, GFP_KERNEL);
    	if(!dma_buffer)
	{
    		printk("[FT6336][TSP] dma_alloc_coherent error\n");
	}
	*/
	//END: fangjie
	i2c_clientft->timing = 400;	   //when in upgrade mode ,the clock shall be changed to 400
	//for(index = 0;index < sizeof(reg_addr)/sizeof(reg_addr[0]);index++)		//add ft6336 bootloader mode test
	for(index = 0; index <= FTS_UPGRADE_LOOP; index++)
	{
		/*write 0xaa to register 0xfc(FT6336) or 0xbc(ft6336)*/
		...
		#endif
		TPD_DMESG("[FT6336-upgrade] Step 1: Reset CTPM test\n");


		/*********Step 2:Enter upgrade mode *****/
		TPD_DMESG("[FT6336-upgrade] Step 2: Enter upgrade mode\n");
		...
		byte_read(reg_val,2);
		TPD_DMESG("[FT6336-upgrade] Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",reg_val[0],reg_val[1]);
		//if (reg_val[0] == 0x79 && reg_val[1] == 0x08) //fangjie modify for FT6336
		if (reg_val[0] == FT6336_UPGRADE_ID_1 && reg_val[1] == FT6336_UPGRADE_ID_2)
		{
		
			TPD_DMESG("[FT6336] check READ-ID Successful ! index = %d------\n", index);
			break;
		}
		else if(index >= FTS_UPGRADE_LOOP)
		{
			TPD_DMESG("[FT6336] check READ-ID faild !\n");
			i2c_clientft->timing = 100;
			return ERR_READID;
		}
	}	
	TPD_DMESG("[FT6336] ---index: %d------\n", index);

	cmd_write(0xcd,0x0,0x00,0x00,1);
	byte_read(reg_val,1);
	TPD_DMESG("[FT6336-upgrade] bootloader version = 0x%x\n", reg_val[0]);
	...
	TPD_DMESG("[FT6336-upgrade] Step 4: erase. \n");

	/*********Step 5:write firmware(FW) to ctpm flash*********/
	bt_ecc = 0;
	TPD_DMESG("[FT6336-upgrade] Step 5: start upgrade. \n");
	dw_lenth = dw_lenth - 8;
	...
	for (j = 0;j < packet_number;j ++)
	{
		...
		{
			TPD_DMESG("[FT6336-upgrade] upgrade the 0x%x th byte.\n", ((unsigned int)j) * FTS_PACKET_LENGTH);
		}
	}
...
int fts_ctpm_auto_clb(void)
{
	...
	TPD_DMESG("[FT6336] start auto CLB.\n");
	msleep(200);
	...
	for(i=0;i<50;i++) //llf 20130220
	{
		...
		delay_qt_ms(200);
		//mtk_wdt_restart(WD_TYPE_NOLOCK);//avoid watchdog timeout
		TPD_DMESG("[FT6336] waiting calibration %d\n",i);
	}
	mtk_wdt_restart(WD_TYPE_NOLOCK);//avoid watchdog timeout
	msleep(200);
	ft5x0x_write_reg(0, 0x40);  
	...
	TPD_DMESG("[FT6336] calibration OK.\n");
	...
	msleep(300);
	TPD_DMESG("[FT6336] store CLB result OK.\n");
...
int fts_ctpm_fw_upgrade_with_i_file(void)
{
	...
	if(tp_vendor == FT_ID_JUNDA){
		pbt_buf = CTPM_FW_PIXI4_JUNDA;
		ui_sz = sizeof(CTPM_FW_PIXI4_JUNDA);
	}
	else
	{
		pbt_buf = CTPM_FW_PIXI4_JUNDA;
	...
	if (i_ret != 0)
	{
		TPD_DMESG("[FT6336] upgrade failed i_ret = %d.\n", i_ret);
		//error handling ...
		//TBD
	}
	else
	{
		TPD_DMESG("[FT6336] upgrade successfully.\n");
...
unsigned char fts_ctpm_get_i_file_ver(void)
{
	...
	if(tp_vendor == FT_ID_JUNDA)
	{
		...
		if (ui_sz > 2)
		{
			return CTPM_FW_PIXI4_JUNDA[0x10a];
		}
		else
		{
...
/*============================================================================*/
/*============================================================================*/
/*================BEGIN: CTP Simulate P-sensor Function===================================*/
...
static int ft6336_pls_opened=0;
static int ft6336_tp_resume_flag=0;//add by PR653624, once the tp reset , should set ft6336_tp_resume_flag=0.s

/*************************************************************************************/
static void ft6336_tp_reset() //add by PR653624
{
	mt_set_gpio_mode(GPIO_CTP_EN_PIN, GPIO_CTP_EN_PIN_M_GPIO);
	mt_set_gpio_dir(GPIO_CTP_EN_PIN, GPIO_DIR_OUT);
	mt_set_gpio_out(GPIO_CTP_EN_PIN, GPIO_OUT_ONE);
	...
	mt_set_gpio_out(GPIO_CTP_EN_PIN, GPIO_OUT_ONE);
	msleep(300);	
	ft6336_tp_resume_flag=0;//add by PR653624
}
static int  ft6336_ps_mode_enable(bool enable)
{	      
	int err = 0;
	...
	{
		TPD_DMESG("[FT6336]TPD wake up befor the PS enable!**************************\n");
		ft6336_tp_reset(); 

		err= ft5x0x_write_reg(0xB0,0x01);
		TPD_DMESG("[FT6336_PS]open ps is %d \n",err);
		mdelay(50);
		err=ft5x0x_read_reg(0xB0,&rx_data);
		if(err>=0 && (rx_data&0x01==1)){
			TPD_DMESG("[FT6336_PS]open ps OK\n");
		}
		else{
			TPD_DMESG("[FT6336_PS]open ps failed,rx_data=%d err=%d \n",rx_data,err);
			return -1;
		}
	}
	else //close phone
	{     
		...
		TPD_DMESG("[FT6336_PS]close ps is %d \n",err);
		mdelay(50);
		err=ft5x0x_read_reg(0xB0,&rx_data);
		if(err>=0 && (rx_data ==0)){
			TPD_DMESG("[FT6336_PS]close ps OK\n");
		}
		else{
			TPD_DMESG("[FT6336_PS]close ps failed,  rx_data=%d err=%d\n",rx_data,err);
		}
	}
	return 0;
}

int ft6336_pls_enable(void)
{
	int ret=0;
	TPD_DMESG("[FT6336_PS]ft6336_pls_enable\n");
	if(ft6336_pls_opened>0){
		TPD_DMESG("[FT6336_PS]ft6336_pls have enable\n");
		return 0;
	}
	ret=ft6336_ps_mode_enable(true);
	if(ret>=0){
		ft6336_pls_opened = 1;
	}
	return 0;
...
int ft6336_pls_disable(void)
{
	TPD_DMESG("[FT6336_PS]ft6336_pls_disable\n");
	if(ft6336_pls_opened<1){
		TPD_DMESG("[FT6336_PS]ft6336_pls have disable\n");
		return 0;
	}
	ft6336_ps_mode_enable(false);
	ft6336_pls_opened = 0;
	return 0;
}	

int  get_ft6336_data(void )
{
	return fps_state;
}

 static int ft6336_pls_open(struct inode *inode, struct file *file)
 {
	 TPD_DMESG("[FT6336_PS] %s\n", __func__);
	 if (ft6336_pls_opened)
		 return -EBUSY;
	 ft6336_pls_opened = 1;
	 return 0;
 }
 static int ft6336_pls_release(struct inode *inode, struct file *file)
 {
	 TPD_DMESG("[FT6336_PS] %s", __func__);
	 ft6336_pls_opened = 0;
	 return 0;//ap3212c_pls_disable(AP3212C_PLS_BOTH);
 }

static int ft6336_pls_unlocked_ioctl(struct file *file, unsigned int cmd,unsigned long arg)   
 {
	 void __user *argp = (void __user *)arg;
	 TPD_DMESG("[FT6336_PS] %s: cmd %d", __func__, _IOC_NR(cmd));
	 switch (cmd) {
	 case 1:
		 TPD_DMESG("[FT6336_PS] %s: FACE_DETECT_NEAR*************************\n", __func__);
		 ft6336_pls_enable();
		 break;
	 case 2:
		 TPD_DMESG("[FT6336_PS] %s: FACE_DETECT_FAR*************************\n", __func__);
		 ft6336_pls_disable();
		 break;
	 default:
		 TPD_DMESG("[FT6336_PS] %s: invalid cmd %d\n", __func__, _IOC_NR(cmd));
		 return -EINVAL;
	 }
	 return 0;
 }
 
 static struct file_operations ft6336_pls_fops = {
	 .owner 		= THIS_MODULE,
	 .open			= ft6336_pls_open,
	 .release		= ft6336_pls_release,
	 .unlocked_ioctl 	= ft6336_pls_unlocked_ioctl,
 };
 static struct miscdevice ft6336_pls_device = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = ALSPS_DEVICE_NAME,
	.fops = &ft6336_pls_fops,
};
#endif


/*================END: CTP Simulate P-sensor Function===================================*/
/*============================================================================*/
/*============================================================================*/
...
/*============================================================================*/
/*============================================================================*/
/*================BEGIN: Upgrade Firmware Function after poweron process========================*/
#define FT_BOOT_UPGRADE_FIRMWARE

#ifdef FT_BOOT_UPGRADE_FIRMWARE
int fts_ctpm_get_panel_factory_setting(void)    //return factory ID
{
	...
	/*********Step 1:Reset  CTPM *****/
	for(j = 0;j < sizeof(reg_addr)/sizeof(reg_addr[0]);j++)		//add ft6336 bootloader mode test
	{
		/*write 0xaa to register 0xfc(FT6336) or 0xbc(ft6336)*/
		...
		TPD_DMESG("[FT6336] Step 1: Reset CTPM test\n");

		delay_qt_ms(delay_time2[j]);

		/*********Step 2:Enter upgrade mode *****/
		TPD_DMESG("[FT6336] Step 2: Enter upgrade mode\n");
		auc_i2c_write_buf[0] = 0x55;
		auc_i2c_write_buf[1] = 0xaa;
		do
		{
		...
		{
			if(reg_val[1] == 0x7)
				TPD_DMESG("[FT6336] Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",reg_val[0],reg_val[1]);
	...
	TPD_DMESG("[FT6336] old setting: uc_i2c_addr = 0x%x, uc_io_voltage = %d, uc_panel_factory_id = 0x%x\n",
	buf[0],  buf[2], buf[4]);
...
int fts_ctpm_auto_upg(unsigned char tp_vendor)
{
	...
	unsigned char version_list_camry_each[] = {0x0,0x10,0x11,0x12,0x13,0x14,0x15,0x16,0x20,0x61};	//Keep the ft6336 tp firmware old version list that allows to be updated.
//	unsigned char version_list_camry_junda[] = {0x0,0x10,0x11,0x12,0x13,0x14,0x15};	//Keep the ft6336 tp firmware old version list that allows to be updated.
	#endif //END: delete by fangjie 
	unsigned char version_list_ft6336_junda_pixi4[] = {0x0,0x10,0x11,0x12,0x13,0x14,0x15,0x16,0x20,0x61};	//Keep the ft6336 tp firmware old version list that allows to be updated.
	int i,i_ret = -1;

	uc_tp_fm_ver = ft5x0x_read_fw_ver();
	TPD_DMESG("[FT6336] uc_tp_fm_ver = %x", uc_tp_fm_ver);

	if(tp_vendor == FT_ID_JUNDA)  
	{
		uc_host_fm_ver = fts_ctpm_get_i_file_ver();
			TPD_DMESG("[FT6336] uc_host_fm_ver = %x", uc_host_fm_ver);

		TPD_DMESG("[FT6336] tp vendor is unkown(0x%x),uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",tp_vendor, uc_tp_fm_ver, uc_host_fm_ver);
		//for(i = 0;i < sizeof(version_list_ft6336_junda_pixi4)/sizeof(version_list_ft6336_each_pixi4[0]); i++)
		//{
			if(uc_tp_fm_ver<uc_host_fm_ver)//llf 20130222
			{
				//if(uc_tp_fm_ver == version_list_ft6336_junda_pixi4[i])  
				//{
					TPD_DMESG("[FT6336] Junda tp firmware have new version\n");
					i_ret = fts_ctpm_fw_upgrade_with_i_file();
					if(!i_ret)
						TPD_DMESG("[FT6336] Each tp firmware upgrade to new version 0x%x\n", uc_host_fm_ver);
					else
						TPD_DMESG("[FT6336] Each tp firmware upgrade failed ret=%d.\n", i_ret);
					//break;
				//}
			}
			else
			{
				TPD_DMESG("[FT6336] current junda tp firmware is the latest version!\n");
				//break;
			}
		//}
	}
	...
	{
		TPD_DMESG("[FT6336] Unknown tp vendor(0x%x).\n", uc_tp_fm_ver);
		return -1;
	}
    return 0;
}
...
#define FT_APK_UPGRADE_FIRMWARE

#ifdef FT_APK_UPGRADE_FIRMWARE
...
static int ft5x0x_GetFirmwareSize(char *firmware_name)
{
	...
	if (IS_ERR(pfile)) {
		...
		printk("[ft5436_llf] Get firmware size error\n");
...
static ssize_t ft5x0x_fwupgradeapp_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
	...
	//ID detect 
	if((tp_vendor != 0x5d) && (tp_vendor != 0x53) && (tp_vendor != 0x80)&& (tp_vendor != FT_ID_JUNDA)&& (tp_vendor != 0x87)){
		//destory #######################################
		mt_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
...
/*============================================================================*/
/*============================================================================*/
/*================BEGIN: Rawdata Test for MMITEST=======================================*/
...
static int fts_debug_write(struct file *filp, const char __user *buff, unsigned long len, void *data)
{
	...
	if (copy_from_user(&writebuf, buff, buflen)) {
		TPD_DMESG("[FT6336] copy from user error,%s\n ", __func__);
		return -EFAULT;
	}
	...
	case PROC_READ_REGISTER:
		writelen = 1;
		TPD_DMESG("[FT6336] %s:register addr=0x%02x\n", __func__, writebuf[1]);
		ret = byte_write( writebuf + 1, writelen);
		if (ret < 0) {
			TPD_DMESG("[FT6336] %s:write iic error\n", __func__);
			return ret;
		}
		break;
	...
	case PROC_WRITE_DATA:                                                   
		TPD_DMESG("[FT6336] %s,PROC_READ_DATA,PROC_WRITE_DATA\n",__func__);
		writelen = len - 1;                                                
		ret = byte_write(writebuf + 1, writelen);            
		if (ret < 0) {                                                     
			TPD_DMESG("[FT6336] %s:write iic error\n", __func__);
...
static int fts_debug_read( char *page, char **start,off_t off, int count, int *eof, void *data )
{
	...
	case PROC_READ_REGISTER:
		...
		if (ret < 0) {
			TPD_DMESG("[FT6336] %s:read iic error\n", __func__);
			return ret;
		} else
			TPD_DMESG("[FT6336] %s:value=0x%02x\n", __func__, buf[0]);
		num_read_chars = 1;
		break;
	case PROC_RAWDATA:
		break;
...
*/
	case PROC_READ_DATA:                                                              
		if(count>255){
			TPD_DMESG("[FT6336] %s,%d:PROC_READ_DATA SIZE OUT OF RANGE 255\n",__func__,__LINE__);
		}
		readlen = count;                                                             
		ret = byte_read( buf, readlen);                        
		if (ret < 0) {                                                               
			TPD_DMESG("[FT6336] %s,%d:read iic error\n",__func__,__LINE__);
		      return ret;                                                            
		}                                                                            
		...
		break;                                                                       
	case PROC_WRITE_DATA:                                                             
		TPD_DMESG("[FT6336] %s,%d:PROC_WRITE_DATA\n",__func__,__LINE__);
    		break;                                                                       
	default:
		break;
	}
...
{
	printk("%s,[FT6336] OPEN MMI TEST \n",__func__);
	fts_proc_entry = create_proc_entry(PROC_NAME, 0777, NULL);
	if (NULL == fts_proc_entry) {
		TPD_DMESG("[FT6336] Create proc entry fail!\n");
		return -ENOMEM;
	} else {
		TPD_DMESG("[FT6336] Create proc entry success!\n");
...
{
	fts_proc_entry1 = create_proc_entry(PROC_NAME1, 0666, NULL);
...
#endif
/*================END: Rawdata Test for MMITEST=======================================*/
/*============================================================================*/
/*============================================================================*/



//BEGIN: add by fangjie for Read firmware version
...
//END: add by fangjie for Read firmware version
                                     ...
                                     struct device_attribute *attr, char *buf)
{
	...
	switch(tp_vendor){
		case FT_ID_MUTTO://mutto
			strid="F1";
			break;
		case FT_ID_TRULY://TRULY
			strid="F2";
			break;
		case FT_ID_EACH://each
			strid="F3";
			break;
		case FT_ID_JUNDA://junda
			strid="F5";
			break;
		default:
			strid="FF";
...
//	if(point_num == 0) return false;
 ...
 static int touch_event_handler(void *unused)
 {
	...
	tpd_flag = 0;	
 	...
 	if(ft6336_pls_opened){
		...
		printk("[FT6336_llf] PS reg data =0x%x \n ",ps_data);
 ...
 {	 
	...
	hwPowerOn(TPD_POWER_SOURCE,VOL_DEFAULT,"TP");
	msleep(100);
#endif
	msleep(5);
	mt_set_gpio_out(GPIO_CTP_EN_PIN, GPIO_OUT_ONE);
...
#ifdef FT_BOOT_UPGRADE_FIRMWARE
	ft5x0x_read_reg(0xA8, &tp_vendor);
	TPD_DMESG("[FT6336] tpd_probe: tp_vendor = %x", tp_vendor);
	if((tp_vendor != 0x5a) && (tp_vendor != 0x53) && (tp_vendor != 0x80)&& (tp_vendor != FT_ID_JUNDA))
	{
		tp_vendor = fts_ctpm_get_panel_factory_setting();
			TPD_DMESG("[FT6336] fts_ctpm_get_panel_factory_setting  tp_vendor= %x", tp_vendor);
	...
	if((tp_vendor != 0x5a) && (tp_vendor != 0x53) && (tp_vendor != 0x80)&& (tp_vendor != FT_ID_JUNDA)){
		//destory #######################################
		mt65xx_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
		TPD_DMESG("[%s] can't find ft6336,vendor:0x%x\n",__func__,tp_vendor);
		return -1;
	}
...
#ifdef CTP_PSENSOR_SUPPORT
	ret = misc_register(&ft6336_pls_device);
	if (ret != 0) {
		printk("cannot register miscdev on ft6336_pls_device\n");
	}
#endif

	//BEGIN: add by fangjie for read firwmare version.
	if(device_create_file(&i2c_clientft->dev, &dev_attr_version)>=0)
	{            
		printk("device_create_file_version \n");        
	}
	//END: add by fangjie for read firwmare version.
...
//  .address_data = &addr_data,
 ...
 {
	TPD_DMESG("[FT6336]--------------------------------------tpd_local_init\n");
	...
	{
		TPD_DMESG("[FT6336] add i2c driver faild.\n");
 ...
 static void tpd_resume( struct early_suspend *h )
 {
	...
	if(ft6336_pls_opened)
	{	
		if(ft6336_tp_resume_flag==0){
			ft6336_tp_reset();
		}
		ft6336_ps_mode_enable(true);//opened by PR653624
		printk("==%s= ft6336_pls_opened ! return\n", __func__);
		mt65xx_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM); 
		return;
	}
#endif
 ...
 static int __init tpd_driver_init(void) {
	TPD_DMESG("MediaTek FT6336 touch panel driver init\n");
	i2c_register_board_info(1, &i2c_tpd, 1);
	if(tpd_driver_add(&tpd_device_driver) < 0)
		TPD_DMESG("add FT6336 driver failed\n");
	return 0;
 }
 
 /* should never be called */
 static void __exit tpd_driver_exit(void) {
	 TPD_DMESG("MediaTek FT6336 touch panel driver exit\n");
